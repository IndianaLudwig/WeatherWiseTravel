...
def parse_places_nearby(place_results) :
    if(place_results['status'] == 'OK') :
        ...
        parse_location_option(place_results, location_name)
            
                
    else :
        print("Error in API callback result")

def parse_location_option(place_results, location_option) :
    if(place_results['status'] == 'OK') :
        for place in place_results['results']:
            if (place['name'] == location_option):                
                my_place_name = place['name']
                my_place_address = place['formatted_address']
                my_place_rating = place['rating']
                #my_place_rating_from = place['types']['user_ratings_total']
                my_place_link = my_place_name.replace(" ", "+")
                my_place_lat = place['geometry']['location']['lat']
                my_place_lng = place['geometry']['location']['lng']
                thisdict = {
                        "name": my_place_name,
                        "address": my_place_address,
                        "co-ordinates": [my_place_lng, my_place_lat],
                        "rating" : my_place_rating,
                        "link" : 'https://maps.google.com/?q=' + my_place_link
                        }
                location_info.append(thisdict)
                '''print("Name: " + my_place_name + '\n' + "Address: " + my_place_address + '\n' + "Rating: "+ str(my_place_rating)
                      + '\n' + 'Link: ' + 'https://maps.google.com/?q=' + my_place_link)'''
